/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 */
package com.mycompany.employeeseries;

//import com.mycompany.employeeseries.version1.*;
//import version2.*;
//import version3.*;
//import version5.EmployeeRoster;
//import version6.*;
import version7.*;
/**
 *
 * @author light
 */
public class EmployeeSeries {

    public static void main(String[] args) {

//        HourlyEmployee h1 = new HourlyEmployee(41, 100, "Clarke", 2310);
//        HourlyEmployee h2 = new HourlyEmployee("Clarke", 2310);
//        HourlyEmployee h3 = new HourlyEmployee();
//
//        System.out.println("----------HOURLY EMPLOYEEE----------");
//        h1.displayHourlyEmployee();
//        System.out.println("----------");
//        h2.displayHourlyEmployee();
//        System.out.println("----------");
//        h3.displayHourlyEmployee();
//
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------COMMISION EMPLOYEEE----------");
//        CommisionEmployee c1 = new CommisionEmployee();
//        CommisionEmployee c2 = new CommisionEmployee("Pablo", 3439);
//        CommisionEmployee c3 = new CommisionEmployee(12300, "Pablo", 3439);
//        c1.displayCommisionEmployee();
//        System.out.println("----------");
//        c2.displayCommisionEmployee();
//        System.out.println("----------");
//        c3.displayCommisionEmployee();
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------PIECE EMPLOYEEE----------");
//        PieceEmployee p1 = new PieceEmployee();
//        PieceEmployee p2 = new PieceEmployee("Jab", 1456);
//        PieceEmployee p3 = new PieceEmployee(500, 1.0, "Jack", 7890);
//
//        p1.displayPieceEmployee();
//        System.out.println("----------");
//        p2.displayPieceEmployee();
//        System.out.println("----------");
//        p3.displayPieceEmployee();
//        System.out.println("----------");
//        System.out.println("----------");
//
//        System.out.println("----------BASE PLUS COMMISSION EMPLOYEEE----------");
//
//        BasedPlusCommissionEmployee b1 = new BasedPlusCommissionEmployee();
//        BasedPlusCommissionEmployee b2 = new BasedPlusCommissionEmployee(10000, 9000, "Cole", 8888);
//        BasedPlusCommissionEmployee b3 = new BasedPlusCommissionEmployee("Neto", 6969);
//        b1.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b2.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b3.displayBasedPlusCommissionEmployee();
//        HourlyEmployee h1 = new HourlyEmployee(41, 100, "Clarke", 2310);
//        HourlyEmployee h2 = new HourlyEmployee("Clarke", 2310);
//        HourlyEmployee h3 = new HourlyEmployee();
//        HourlyEmployee h4 = new HourlyEmployee(41, 100);
//        HourlyEmployee h5 = new HourlyEmployee("Clarke");
//        System.out.println("----------HOURLY EMPLOYEEE----------");
//        h1.displayHourlyEmployee();
//        System.out.println("----------");
//        h2.displayHourlyEmployee();
//        System.out.println("----------");
//        h3.displayHourlyEmployee();
//        System.out.println("----------");
//        h4.displayHourlyEmployee();
//        System.out.println("----------");
//        h5.displayHourlyEmployee();
//
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------COMMISION EMPLOYEEE----------");
//        CommisionEmployee c2 = new CommisionEmployee("Pablo", 3439);
//        CommisionEmployee c3 = new CommisionEmployee(12300, "Pablo", 3439);
//        CommisionEmployee c4 = new CommisionEmployee(12300);
//        CommisionEmployee c5 = new CommisionEmployee("Pablo");
//        System.out.println("----------");
//        c2.displayCommisionEmployee();
//        System.out.println("----------");
//        c3.displayCommisionEmployee();
//        System.out.println("----------");
//        c4.displayCommisionEmployee();
//        System.out.println("----------");
//        c5.displayCommisionEmployee();
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------PIECE EMPLOYEEE----------");
//        PieceEmployee p1 = new PieceEmployee();
//        PieceEmployee p2 = new PieceEmployee("Jab", 1456);
//        PieceEmployee p3 = new PieceEmployee(500, 1.0, "Jack", 7890);
//        PieceEmployee p4 = new PieceEmployee(500, 1.0);
//        PieceEmployee p5 = new PieceEmployee("Jack");
//
//        p1.displayPieceEmployee();
//        System.out.println("----------");
//        p2.displayPieceEmployee();
//        System.out.println("----------");
//        p3.displayPieceEmployee();
//        System.out.println("----------");
//        p4.displayPieceEmployee();
//        System.out.println("----------");
//        p5.displayPieceEmployee();
//        System.out.println("----------");
//        System.out.println("----------");
//
//        System.out.println("----------BASE PLUS COMMISSION EMPLOYEEE----------");
//
//        BasedPlusCommissionEmployee b1 = new BasedPlusCommissionEmployee();
//        BasedPlusCommissionEmployee b2 = new BasedPlusCommissionEmployee(10000, 9000, "Cole", 8888);
//        BasedPlusCommissionEmployee b3 = new BasedPlusCommissionEmployee("Neto", 6969);
//        BasedPlusCommissionEmployee b4 = new BasedPlusCommissionEmployee(10000, "Cole", 8888);
//        BasedPlusCommissionEmployee b5 = new BasedPlusCommissionEmployee(10000);
//        b1.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b2.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b3.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b4.displayBasedPlusCommissionEmployee();
//        System.out.println("----------");
//        b5.displayBasedPlusCommissionEmployee();
//
//        HourlyEmployee h1 = new HourlyEmployee(30, 120, "Clarke", "Hubahib", "Canal", 2310, 5, 12, 2005, 10, 2, 2024);
//        HourlyEmployee h2 = new HourlyEmployee(30, 120, "Clarke", "Hubahib", "Canal", 2315, 12, 12, 2012);
//        HourlyEmployee h3 = new HourlyEmployee();
//        HourlyEmployee h4 = new HourlyEmployee(41, 100);
//        HourlyEmployee h5 = new HourlyEmployee(30, 120, 12, 12, 2012);
//        HourlyEmployee h6 = new HourlyEmployee(30, 120, "Clarke", "Canal", 2317, 5, 12, 2005, 10, 2, 2024);
        
        
//        EmployeeRoster r = new EmployeeRoster(10);
         
//       
//        r.addEmployee(h1);
//        r.addEmployee(h2);
//        r.addEmployee(h6);
                 
//        System.out.println("----------HOURLY EMPLOYEEE----------");
//        h1.display();
//        System.out.println("----------");
//        h2.display();
//        System.out.println("----------");
//        h3.display();
//        System.out.println("----------");
//        h4.display();
//        System.out.println("----------");
//        h5.display();
//        System.out.println("----------");
//        h6.display();
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------COMMISION EMPLOYEEE----------");
//        CommisionEmployee c1 = new CommisionEmployee();
//         CommisionEmployee c2 = new CommisionEmployee(12300, "Pablo", "blo", "jab", 3439, 12, 5, 2012, 10, 2, 2024);
//         r.Add(c2);
//         System.out.println(r.countCE());
//        r.addEmployee(c2);
//        System.out.println(r.countHE());    
//         System.out.println(r.countCE());
//        CommisionEmployee c3 = new CommisionEmployee(12300, 12, 5, 2012);
//        CommisionEmployee c4 = new CommisionEmployee(12300, "Pablo", "blo", "jab", 3439, 2012, 12, 12);
//        CommisionEmployee c5 = new CommisionEmployee(12300);
//        CommisionEmployee c6 = new CommisionEmployee(12300, "Pablo", "jab", 3439, 12, 5, 2012, 10, 2, 2024);
//        c1.display();
//        System.out.println("----------");
//        c2.display();
//        System.out.println("----------");
//        c3.display();
//        System.out.println("----------");
//        c4.display();
//        System.out.println("----------");
//        c5.display();
//        System.out.println("----------");
//        c6.display();
//        System.out.println("----------");
//        System.out.println("----------");
//        System.out.println("----------PIECE EMPLOYEEE----------");
//        PieceEmployee p1 = new PieceEmployee();
//        PieceEmployee p2 = new PieceEmployee("Jab", "Baj", "AJB", 1456, 12, 12, 2012);
//        PieceEmployee p3 = new PieceEmployee(500, 1.0, "Jack", "Cole", "Lin", 7890, 3, 3, 2000, 12, 12, 2012);
//        PieceEmployee p4 = new PieceEmployee(500, 1.0, "Jack", "Cole", "Lin", 7890, 12, 12, 2012);
//        PieceEmployee p5 = new PieceEmployee(5000, 20.0);
//        PieceEmployee p6 = new PieceEmployee(5000, 20.0, 12, 12, 2012);
//        PieceEmployee p7 = new PieceEmployee(500, 1.0, "Jack", "Lin", 7890, 3, 3, 2000, 12, 12, 2012);
//        p1.display();
//        System.out.println("----------");
//        p2.display();
//        System.out.println("----------");
//        p3.display();
//        System.out.println("----------");
//        p4.display();
//        System.out.println("----------");
//        p5.display();
//        System.out.println("----------");
//        p6.display();
//        System.out.println("----------");
//        p7.display();
//
//        System.out.println("----------");
//        System.out.println("----------");
//////
//        System.out.println("----------BASE PLUS COMMISSION EMPLOYEEE----------");
//
//        BasedPlusCommissionEmployee b1 = new BasedPlusCommissionEmployee();
//        BasedPlusCommissionEmployee b2 = new BasedPlusCommissionEmployee(10000);
//        BasedPlusCommissionEmployee b3 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
//        BasedPlusCommissionEmployee b4 = new BasedPlusCommissionEmployee(10000, 9000, 12, 12, 1998);
//        BasedPlusCommissionEmployee b5 = new BasedPlusCommissionEmployee(10000, 9000, "Neto", "Talib", "Otob", 6969, 2, 14, 2002);
//        BasedPlusCommissionEmployee b6 = new BasedPlusCommissionEmployee(10000, 9000);
//        BasedPlusCommissionEmployee b7 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
//           r.Add(b3);
//           r.Add(b7);
////         r.addEmployee(b3);
////         r.addEmployee(b7);
//          System.out.println(r.countBPCE());
          
          
//         
        //   r.Display();
//        b1.display();
//        System.out.println("----------");
//        b2.display();
//        System.out.println("----------");
//        b3.display();
//        System.out.println("----------");
//        b4.display();
//        System.out.println("----------");
//        b5.display();
//        System.out.println("----------");
//        b6.display();
//        System.out.println("----------");
//        b7.display();
//---------------------------VERSION 5-----------------------------------------------------------
    //       HourlyEmployee h1 = new HourlyEmployee(30, 120, "Clarke", "Hubahib", "Canal", 2310, 5, 12, 2005, 10, 2, 2024);
    //       HourlyEmployee h2 = new HourlyEmployee(30, 120, "Clarkes", "Gwapo", "asda", 2311, 5, 13, 2005, 10, 2, 2024);
    //       HourlyEmployee h3 = new HourlyEmployee(30, 120, "sere", "qwer", "asdd", 2312, 5, 14, 2005, 10, 2, 2024);
    //       HourlyEmployee h4 = new HourlyEmployee(30, 120, "gews", "Hvar", "dsdssd", 2313, 5, 15, 2005, 10, 2, 2024);
    //       
    //       CommisionEmployee c4 = new CommisionEmployee(12300, "Pablos", "blo", "jabs", 3440, 12, 5, 2012, 10, 2, 2024);
    //       CommisionEmployee c5 = new CommisionEmployee(12300, "Pablod", "blo", "jabd", 3441, 12, 5, 2012, 10, 2, 2024);
    //       CommisionEmployee c6 = new CommisionEmployee(12300, "Pablo1", "blo", "jab2", 3442, 12, 5, 2012, 10, 2, 2024);
    //       
    //       PieceEmployee p4 = new PieceEmployee(500, 1.0, "Jacks", "Coles", "Lins", 7891, 3, 4, 2000, 12, 12, 2012);
    //       PieceEmployee p5 = new PieceEmployee(500, 1.0, "Jackasd", "Coled", "Line", 7892, 3, 5, 2000, 12, 12, 2012);
    //       PieceEmployee p6 = new PieceEmployee(500, 1.0, "Jackwee", "Coleb", "Linqw", 7893, 3, 6, 2000, 12, 12, 2012);
    //       PieceEmployee p7 = new PieceEmployee(500, 1.0, "Jackdsdx", "Coleasd", "Linds", 7894, 3, 7, 2000, 12, 12, 2012);
    //       
    //       
    //       BasedPlusCommissionEmployee b4 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //       BasedPlusCommissionEmployee b5 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //       BasedPlusCommissionEmployee b6 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //        EmployeeRoster r = new EmployeeRoster();
    //          r.Add(h1);
    //          r.Add(h2);
    //          r.Add(h3);
    //          r.Add(h4);
    //          r.Add(c6);
    //          r.Add(c4);
    //          r.Add(c5);
    //          r.Add(p7);
    //          r.Add(p4);
    //          r.Add(p5);
    //          r.Add(p6);
    //          r.Add(b4);
    //          r.Add(b5);
    //          r.Add(b6);
    //          
    //          System.out.println("NUMBER OF HOURLY EMPLOYEES: "+r.countHE());
    //          System.out.println("NUMBER OF COMMISION EMPLOYEES: " + r.countCE());
    //          System.out.println("SNUMBER OF PIECEWORKERS: " + r.countPE());
    //          System.out.println("NUMBER OF BASEDPLUSCOMMISION EMPLOYESS: " + r.countBPCE());
    //          
    //          r.displayHE();
    //          r.displayCE();
    //          r.displayPWE();
    //          r.displayBPCE();
    //          
    //          r.displayAllEmployee();
 //---------------------------VERSION 6----------------------------------------------------------- 
 
    //  HourlyEmployee h1 = new HourlyEmployee(30, 120, "Clarke", "Hubahib", "Canal", 2310, 5, 12, 2005, 10, 2, 2024);
    //       HourlyEmployee h2 = new HourlyEmployee(30, 120, "Clarkes", "Gwapo", "asda", 2311, 5, 13, 2005, 10, 2, 2024);
    //       HourlyEmployee h3 = new HourlyEmployee(30, 120, "sere", "qwer", "asdd", 2312, 5, 14, 2005, 10, 2, 2024);
    //       HourlyEmployee h4 = new HourlyEmployee(30, 120, "gews", "Hvar", "dsdssd", 2313, 5, 15, 2005, 10, 2, 2024);
    //       
    //       CommisionEmployee c4 = new CommisionEmployee(12300, "Pablos", "blo", "jabs", 3440, 12, 5, 2012, 10, 2, 2024);
    //       CommisionEmployee c5 = new CommisionEmployee(12300, "Pablod", "blo", "jabd", 3441, 12, 5, 2012, 10, 2, 2024);
    //       CommisionEmployee c6 = new CommisionEmployee(12300, "Pablo1", "blo", "jab2", 3442, 12, 5, 2012, 10, 2, 2024);
    //       
    //       PieceEmployee p4 = new PieceEmployee(500, 1.0, "Jacks", "Coles", "Lins", 7891, 3, 4, 2000, 12, 12, 2012);
    //       PieceEmployee p5 = new PieceEmployee(500, 1.0, "Jackasd", "Coled", "Line", 7892, 3, 5, 2000, 12, 12, 2012);
    //       PieceEmployee p6 = new PieceEmployee(500, 1.0, "Jackwee", "Coleb", "Linqw", 7893, 3, 6, 2000, 12, 12, 2012);
    //       PieceEmployee p7 = new PieceEmployee(500, 1.0, "Jackdsdx", "Coleasd", "Linds", 7894, 3, 7, 2000, 12, 12, 2012);
    //       
    //       
    //       BasedPlusCommissionEmployee b4 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //       BasedPlusCommissionEmployee b5 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //       BasedPlusCommissionEmployee b6 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
    //        EmployeeRoster r = new EmployeeRoster();
    //          r.Add(h1);
    //          r.Add(h2);
    //          r.Add(h3);
    //          r.Add(h4);
    //          r.Add(c6);
    //          r.Add(c4);
    //          r.Add(c5);
    //          r.Add(p7);
    //          r.Add(p4);
    //          r.Add(p5);
    //          r.Add(p6);
    //          r.Add(b4);
    //          r.Add(b5);
    //          r.Add(b6);
    //          
    //          System.out.println("NUMBER OF HOURLY EMPLOYEES: "+r.countHE());
    //          System.out.println("NUMBER OF COMMISION EMPLOYEES: " + r.countCE());
    //          System.out.println("SNUMBER OF PIECEWORKERS: " + r.countPE());
    //          System.out.println("NUMBER OF BASEDPLUSCOMMISION EMPLOYESS: " + r.countBPCE());
    //          
    //          r.displayHE();
    //          r.displayCE();
    //          r.displayPWE();
    //          r.displayBPCE();
    //          
    //          r.displayAllEmployee();
          
 //---------------------------VERSION 7----------------------------------------------------------- 
 
 
       HourlyEmployee h1 = new HourlyEmployee(30, 120, "Clarke", "Hubahib", "Canal", 2310, 5, 12, 2005, 10, 2, 2024);
       HourlyEmployee h2 = new HourlyEmployee(30, 120, "Clarkes", "Gwapo", "asda", 2311, 5, 13, 2005, 10, 2, 2024);
       HourlyEmployee h3 = new HourlyEmployee(30, 120, "sere", "qwer", "asdd", 2312, 5, 14, 2005, 10, 2, 2024);
       HourlyEmployee h4 = new HourlyEmployee(30, 120, "gews", "Hvar", "dsdssd", 2313, 5, 15, 2005, 10, 2, 2024);
       
       CommisionEmployee c4 = new CommisionEmployee(12300, "Pablos", "blo", "jabs", 3440, 12, 5, 2012, 10, 2, 2024);
       CommisionEmployee c5 = new CommisionEmployee(12300, "Pablod", "blo", "jabd", 3441, 12, 5, 2012, 10, 2, 2024);
       CommisionEmployee c6 = new CommisionEmployee(12300, "Pablo1", "blo", "jab2", 3442, 12, 5, 2012, 10, 2, 2024);
       
       PieceEmployee p4 = new PieceEmployee(500, 1.0, "Jacks", "Coles", "Lins", 7891, 3, 4, 2000, 12, 12, 2012);
       PieceEmployee p5 = new PieceEmployee(500, 1.0, "Jackasd", "Coled", "Line", 7892, 3, 5, 2000, 12, 12, 2012);
       PieceEmployee p6 = new PieceEmployee(500, 1.0, "Jackwee", "Coleb", "Linqw", 7893, 3, 6, 2000, 12, 12, 2012);
       PieceEmployee p7 = new PieceEmployee(500, 1.0, "Jackdsdx", "Coleasd", "Linds", 7894, 3, 7, 2000, 12, 12, 2012);
       
       
       BasedPlusCommissionEmployee b4 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
       BasedPlusCommissionEmployee b5 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
       BasedPlusCommissionEmployee b6 = new BasedPlusCommissionEmployee(10000, 9000, "Kans", "thost", "Tans", 8888, 12, 12, 1998, 10, 2, 2024);
        EmployeeRoster r = new EmployeeRoster();
          r.Add(h1);
          r.Add(h2);
          r.Add(h3);
          r.Add(h4);
          r.Add(c6);
          r.Add(c4);
          r.Add(c5);
          r.Add(p7);
          r.Add(p4);
          r.Add(p5);
          r.Add(p6);
          r.Add(b4);
          r.Add(b5);
          r.Add(b6);
          
          System.out.println("NUMBER OF HOURLY EMPLOYEES: "+r.countHE());
          System.out.println("NUMBER OF COMMISION EMPLOYEES: " + r.countCE());
          System.out.println("SNUMBER OF PIECEWORKERS: " + r.countPE());
          System.out.println("NUMBER OF BASEDPLUSCOMMISION EMPLOYESS: " + r.countBPCE());
          
          r.displayHE();
          r.displayCE();
          r.displayPWE();
          r.displayBPCE();
          
          r.displayAllEmployee();
    }
}
